...
export function ExpenseItem({ expense, userId, memberProfiles, settled, onSettle }: ExpenseItemProps) {
  ...
  return (
    <div className="bg-card rounded-[14px] border-[0.5px] border-[rgba(44,40,32,0.07)] px-4 py-3"
      style={{ opacity: settled ? 0.5 : 1 }}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-[14px] font-[500] text-ink truncate">{expense.title}</p>
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-[11px] text-ink3">· {expense.date}</span>
          </div>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-[14px] font-[500] text-ink">{formatCurrency(expense.total)}</p>
          {mySplit && (
            <p className="text-[11px]" style={{ color: iAmPayer ? "#6A9E7A" : "#D4645A" }}>
      ...
      {!iAmPayer && mySplit && !settled && onSettle && (
        <button
          onClick={onSettle}
          className="mt-3 text-[12px] font-[500] text-accent"
        >
          Mark as settled
        </button>
      )}
